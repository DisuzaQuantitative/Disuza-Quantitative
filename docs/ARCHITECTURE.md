...
#### On-Chain Analytics
- **Sources**: Blockchain analytics providers (Glassnode, etc.)
...
|-------------------------|--------------------|----------------------------------------|
| Data source outage      | Health checks      | Failover to backup, cached data        |
| Model service crash     | Liveness probe     | Auto-restart, fallback to prev version |
| Database unavailable    | Connection timeout | Retry with backoff, read from replica  |
| Execution venue down    | Order rejection    | Route to alternate venue               |
| Airflow task failure    | Task state         | Retry policy, alerting                 |

### Recovery Mechanisms

1. **State Persistence**: All critical state persisted to durable storage
2. **Idempotent Operations**: All operations designed for safe retry
3. **Circuit Breakers**: Prevent cascade failures across services
4. **Health-Based Routing**: Automatic traffic shifting from unhealthy instances
5. **DAG Retry Policies**: Configurable retries with exponential backoff
